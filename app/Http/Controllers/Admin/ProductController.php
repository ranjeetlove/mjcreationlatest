    ...
    while (($line = fgetcsv($file)) !== FALSE) {
        if ($i != 1) {
            ...
            if (!Product::where('sku', $line[0])->exists()) {
                ...
                $categoryName = trim($line[1]);
                $subcategoryName = trim($line[2] ?? '');
                $childcategoryName = trim($line[3] ?? '');
                $data = new Product;
                $sign = Currency::where('is_default', '=', 1)->first();
                $input['type'] = 'Physical';
                $input['sku'] = $line[0];
                ...
                $input['childcategory_id'] = null;
                 $mcat = Category::where('name', 'like', '%' . $categoryName . '%')->first();

                 if ($mcat) {
                     $input['category_id'] = $mcat->id;

                     if (!empty($subcategoryName)) {
                         $scat = Subcategory::where('name', 'like', '%' . $subcategoryName . '%')->first();
                         if ($scat) {
                     ...
                     if (!empty($childcategoryName)) {
                         $chcat = Childcategory::where('name', 'like', '%' . $childcategoryName . '%')->first();
                         if ($chcat) {
                    ...
                    $input['photo'] = $line[5];
                    $input['name'] = $line[4];
                    $input['slug'] = Str::slug($line[4], '-') . '-' . strtolower($line[0]);
...
public function importImage(){
    ...
    $cats = Category::all();
    $sign = Currency::where('is_default','=',1)->first();
    return view('admin.product.productimgaecsv',compact('cats','sign'));
}

public function importImageSubmit(Request $request)
{
    $uploadedImages = [];
    $existingImages = [];
        ...
        foreach ($files as $file) {
            ...
            if (in_array($extension, $allowedExtensions)) {
                ...
                if (!file_exists($filePath)) {
                  ...
                  $productPath = public_path('assets/images/products/' . $filename);
